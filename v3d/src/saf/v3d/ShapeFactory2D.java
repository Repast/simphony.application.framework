...
public class ShapeFactory2D implements CanvasListener {
	 ...
	 * VSpatials that display this image can then be retrieved with getNamedSpatial.
	 * 
	 * 
	...
	/**
	 * Registers the image at the specified path using the specified name. The image
	 * is registered as a "NamedSpatial." VSpatials that display this image can then
	 * be retrieved with getNamedSpatial.
	 * 
	 * 
	 * @param name the name to register the image under
	 * @param img  the image
	 ...
	 * @return true if any "named spatials" have been registered under the specified
	 ...
	 * 
	 * @param path  the path to the image
	 * @param scale the amount to scale the image by default
	 * 
	 * @return the created VImage2D
	 * @throws IOException if the image file cannot be found or read
	 */
	...
	/**
	 * Creates a VImage2D from the specified BufferedImage. Images are cached so use
	 * the same id to reuse the image data.
	 * 
	...
	public VShape createRectangle(int width, int height, boolean colorUpdatable) {
		return createShape(new Rectangle2D.Float(-width / 2f, -height / 2f, width, height), colorUpdatable);
	}
	 ...
	 * 
	 * @param shape the shape to create the VSpatial for
	 * 
	 * @return the created VSpatial.
	 */
	 ...
	 * 
	 * @param shape           the shape to create the VSpatial for
	 * @param colorUpdateable whether or not the color of this shape can be changed
	 *                        after the first set.
	 * 
	 * @return the created VSpatial.
	 */
	 ...
	 * dispose(GLAutoDrawable) which is called when the opengl canvas associated
	 * with the drawable has been disposed.
	 */
	public void dispose() {
	...
	public void dispose(GLAutoDrawable drawable) {
		...
		if (drawable.getContext() != null && doDispose) {
			//System.out.println(this.toString() + ":dispose(GLAutoDrawable)");
	...
	/**
	 * Creates a circle VShape with the specified radius and the specified number of
	 * slices. The number of slices is the number of "pie" shaped slices to create
	 * the circle out of.
	 * 
	 * @param radius the radius of the circle
	 * @param slices the number of slices to create the circle out of
	 * 
	 ...
	 * @param radius         the radius of the circle
	 * @param slices         the number of slices to create the circle out of
	 * @param colorUpdatable whether or not the color of this circle is updatable
	 ...
	 * (non-Javadoc)
	 * 
	 * @see saf.v3d.CanvasListener#reshape(com.jogamp.opengl.GLAutoDrawable, float,
	 * float, saf.v3d.scene.VRoot)
	 */
	@Override
